    ...
    <div className="relative w-full h-[400px] sm:h-[500px] md:h-[560px] map-wrapper" data-style={mapStyle}>
      ...
      >
        ...
        {filtered.map((f, i) => {
          ...
          return (
            ...
            >
              <Tooltip>
                <div style={{ background: '#080d18', border: '1px solid rgba(251,146,60,0.25)', borderRadius: 10, padding: '8px 12px' }}>
                  <div style={{ color: '#fb923c', fontWeight: 700, fontSize: 12 }}>{f.state || 'India'}</div>
                  <div style={{ color: '#fbbf24', fontFamily: 'monospace', fontWeight: 800, fontSize: 14 }}>FRP: {f.frp || 0} MW</div>
                  <div style={{ color: '#64748b', fontSize: 10 }}>{f.type || 'Fire'} · {f.source || 'MODIS/VIIRS'}</div>
                  {f.confidence && <div style={{ color: '#64748b', fontSize: 10 }}>Conf: {f.confidence}%</div>}
                  {f.date && <div style={{ color: '#64748b', fontSize: 10 }}>{f.date} {f.time || ''}</div>}
                  {f.isModeled && <div style={{ color: '#f59e0b', fontSize: 9 }}>⚠ Modeled estimate</div>}
                </div>
              </Tooltip>
            </CircleMarker>
          );
          ...
          <button key={t} onClick={() => setTypeFilter(t)}
            ...
            className={`px-2.5 py-1 rounded-lg text-[10px] font-bold font-mono uppercase tracking-wide transition-all ${
              typeFilter === t
                ...
                : 'text-slate-500 border border-white/5 bg-[#080d18]/85 hover:border-white/15'
            }`}>
            {t}
          </button>
        ))}
      ...
      <MapPanel className="absolute bottom-3 left-3 z-[1000] p-3.5 rounded-2xl text-xs w-48">
        ...
        <div className="mt-2 pt-2 border-t border-white/5">
          <div className="text-[9px] text-slate-500 font-mono mb-1">Confidence ring</div>
      ...
      <AnimatePresence>
        {selected && (
          <motion.div
            ...
            className="absolute top-16 right-3 z-[1002] bg-[#080d18]/95 border p-4 w-60 shadow-2xl backdrop-blur-lg rounded-2xl"
            style={{ borderColor: `${getFireColor(selected.frp)}50` }}
          >
            <div className="relative pr-6">
              <MapCloseButton onClick={() => setSelected(null)} label="Close fire details" />
              <div className="font-extrabold text-white text-base tracking-tight">{selected.state || 'India'}</div>
              <div className="text-xs font-mono text-amber-400 font-bold mb-3">{selected.type || 'Fire detection'}</div>
            </div>
            <div className="space-y-2 text-xs border-t border-white/5 pt-2.5">
              <div className="flex justify-between"><span className="text-slate-500">FRP</span><span className="text-amber-400 font-mono font-bold">{selected.frp || 0} MW</span></div>
              {selected.brightness && <div className="flex justify-between"><span className="text-slate-500">Brightness</span><span className="text-white font-mono">{selected.brightness} K</span></div>}
