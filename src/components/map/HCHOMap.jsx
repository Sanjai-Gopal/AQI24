    ...
    <div className="relative w-full h-[400px] sm:h-[500px] md:h-[560px] map-wrapper" data-style={mapStyle}>
      ...
      <MapPanel className="absolute bottom-3 left-3 z-[1000] p-3.5 rounded-2xl text-xs w-52">
        ...
        <div className="text-slate-600 mt-2 text-[9px] border-t border-white/5 pt-2 font-mono">
          {selectedYear === 'Live' ? 'Sensor: Sentinel-5P TROPOMI' : `Modeled · ${selectedYear}`}
          <br />Units: molecules/cm²
        </div>
      </MapPanel>

      {/* Status badge */}
      <MapPanel className="absolute bottom-3 right-3 z-[1000] px-3 py-1.5 rounded-lg text-[10px] font-mono text-cyan-400 flex items-center gap-2">
        <span className="w-1.5 h-1.5 bg-cyan-400 rounded-full animate-ping" aria-hidden="true" />
        {safeHotspots.length} hotspots · {selectedYear === 'Live' ? 'LIVE 2026' : selectedYear}
      </MapPanel>
        ...
        {selected && (
          ...
          >
            ...
            <div className="space-y-2 text-xs border-y border-white/5 py-2.5">
              ...
              {selected.isModeled && <div className="text-[10px] text-amber-400 font-mono">⚠ Modeled — not observed</div>}
            </div>
